...
from .models import LibraryBook, UserBook, Search, LibraryTransaction, Book, BookBorrowRecord
...
# Backward compatibility admin classes
class BookAdminLegacy(admin.ModelAdmin):
    """Backward compatibility admin for Book model"""
...
class BookBorrowRecordAdminLegacy(admin.ModelAdmin):
    """Backward compatibility admin for BookBorrowRecord model"""
    
    list_display = ['book_title', 'user_name', 'borrowed_date', 'due_date', 'status', 'fine_amount']
    list_filter = ['status', 'borrowed_date', 'due_date']
    search_fields = ['book__title', 'user__username', 'user__email']
    readonly_fields = ['borrowed_date', 'created_at', 'updated_at']
    ...
    def get_queryset(self, request):
        """Filter by user's school and borrowed books only"""
        qs = super().get_queryset(request)
        qs = qs.filter(type='BORROWED')  # Only show borrowed books
        if not request.user.is_superuser and hasattr(request.user, 'school'):
            qs = qs.filter(book__school=request.user.school)
        return qs

# Register the legacy admins only if needed
# admin.site.register(Book, BookAdminLegacy)
# admin.site.register(BookBorrowRecord, BookBorrowRecordAdminLegacy)
        return qs
